...
class SciAgent(AgentBase):
    ...
    def prompt_reply(self, x: Optional[Union[Msg, Sequence[Msg]]] = None, use_RAG = True, add_memory: bool = True, use_memory = True) -> Msg:
        ...
        # if self.memory:
        #     # 获取历史记忆
        #     history = self.memory.get_memory(recent_n=self.recent_n_mem_for_retrieve)
        #     # 将历史记忆和当前输入的提示词合并
        #     history = self.format_msg(history, x)
        #     query = ("\n".join([msg["content"] for msg in history],) if isinstance(history, list) else str(history))
